...
Sprite.prototype.draw = function(context)
{
	...
	if(this.angle == 0)
	{
		if(this.ifCut)
		{
			...
			context.drawImage(this.img,this.sx,this.sy,this.width,this.height,this.x,this.y,this.width/2,this.height/2);
		}else{
			//不需要截取 直接画
		...
		{
			//其实最后两个参数代表画出来图的大小 用于缩放 我们不需要缩放 所以直接用截取的大小
			context.drawImage(this.img,this.sx,this.sy,this.width,this.height,-this.halfWidth,-this.halfHeight,this.width/2,this.height/2);
		}else{
			//不需要截取 直接画
...
Sprite.prototype.drawWithAngle = function(context,angle,centerx,centery)
{
	...
	if(angle == 0)
	{
		...
		{
			//其实最后两个参数代表画出来图的大小 用于缩放 我们不需要缩放 所以直接用截取的大小
			context.drawImage(this.img,this.sx,this.sy,this.width,this.height,this.x-centerx,this.y-centery,this.width/2,this.height/2);
		}else{
			//不需要截取 直接画
 ...
 * 背景 继承sprite
 */
BGD =function(img,sx,sy,sw,sh,ifCut,x,y,angle)
{
	Sprite.call(this,img,sx,sy,sw,sh,ifCut,x,y,angle);
};
BGD.prototype = new Sprite();
BGD.prototype.constructor = BGD;
BGD.prototype.update = function(game)
{
	if(game.status==WELCOME || game.status==PLAYING)
	{
		this.x -= game.option.groundMovePx1;
		if(this.x<-(BGGROUND_OFFSET-CANVAS_WIDTH))
		{
			this.x = 0;
		}
	}	
...
TubeQueue.prototype.update = function(game)
{
	...
	if(this.tubesX.length>0)
	{
		...
		{			
			...
			game.score++;
			this.tubesX.shift();
            //Add by Xuhao for comfirm that which logo has been collected
            if(game.logoAppearStr.indexOf('|' + game.score + '|')!=-1){
                //lighting up logo under page
                game.logoPosStr = game.logoPosStr.substr(1);
                var logoNeedsLight = game.logoPosStr.substr(0,game.logoPosStr.indexOf('|'));
                game.logoPosStr = game.logoPosStr.substr(logoNeedsLight.length);
...
TubeQueue.prototype.addTube =function()
{
    //记录游戏总数
    game.tatolTubes++;
    console.log(game.tatolTubes + '----' + game.logoAppearStr);
    

    
     if(game.logoAppearStr.indexOf('|' + game.tatolTubes + '|')!=-1){  
        ...
        if(game.logoPosStr.length<=1){
            ...
            if(game.logoPosStr.substr(1,1)==1){
                
                if(getUAType()==4&&getAppType()==2){
                    var tube2 = new Tube(TubeImg27,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                    tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                    this.tubes.push(tube2);
                }else{
                    var tube2 = new Tube(TubeImg21,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                    tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                    this.tubes.push(tube2);
                }
            ...
            if(game.logoPosStr.substr(1,1)==2){
                var tube2 = new Tube(TubeImg22,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                this.tubes.push(tube2);
            }
            
            if(game.logoPosStr.substr(1,1)==3){
                var tube2 = new Tube(TubeImg23,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                this.tubes.push(tube2);
            }
            
            if(game.logoPosStr.substr(1,1)==4){
                var tube2 = new Tube(TubeImg24,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                this.tubes.push(tube2);
            }
            
            if(game.logoPosStr.substr(1,1)==5){
                var tube2 = new Tube(TubeImg25,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                this.tubes.push(tube2);
            }
            
            if(game.logoPosStr.substr(1,1)==6){
                var tube2 = new Tube(TubeImg26,0,0,TUBE_WIDTH,TUBE_HEIGHT,false,CANVAS_WIDTH+TUBE_DELAY,0,0);
                tube2.y = tube1.y + TUBE_HEIGHT + TUBE_DISTY;
                this.tubes.push(tube2);
            }
            
            console.log("+++:" + game.logoPosStr.substr(1,1));
//            console.log("+++:" + game.logoPosStr);
//            game.logoPosStr =  game.logoPosStr.substr(2,game.logoAppearStr.length);
//            console.log("+++:" + game.logoPosStr);
...
Option = function()
{
    ...
    this.groundMovePx1 = CANVAS_WIDTH*this.xSpeed/this.FPS/20;
...
Game = function()
{
    ...
    this.tatolTubes = 0;
    //Add By Xuhao for Random Tube Creation
    this.logoAppearStr = '|';
    this.logoPosStr = '|1|';
    for(var x=1;x<6;x++){
        var tempnum = Math.floor(Math.random()*(20+1)+1);
        while(this.logoAppearStr.indexOf('|' + tempnum + '|')!=-1||tempnum<1||tempnum>20){
            tempnum = parseInt(Math.random()*(20+1)+1);
        }
        this.logoAppearStr = this.logoAppearStr + tempnum + '|';
        
        var tempLogoPos = Math.floor(Math.random()*(7)+1);
        while(this.logoPosStr.indexOf('|' + tempLogoPos + '|')!=-1||tempLogoPos<1||tempLogoPos>6){
            tempLogoPos = Math.floor(Math.random()*(7)+1);
        }
        this.logoPosStr = this.logoPosStr + tempLogoPos + '|';
    }
...
Game.prototype.loop = function()
{
	...
	if(this.status == END)
	{
        ...
        //记录死亡次数
        if(storage.getItem('dead_times')>=0){
            var dt = parseInt(storage.getItem('dead_times'));
            dt = dt+1;
            storage.setItem('dead_times',dt);
        }else{
            storage.setItem('dead_times',0);
        }
            
        if(hightestScore<=this.score){
            console.log('highs:' + hightestScore + ' curr:' + this.score);
            hightestScore = this.score;
